...
const formTitle = computed(() => (editingId.value ? "Edit" : "New"));
...
const validationError = computed(() => {
  ...
  if (!form.match_model.trim()) return "match_model required";
  if (!Number.isInteger(form.priority)) return "priority integer only";
  return null;
});
...
useWs((event: unknown) => {
  const ev = event as { type?: string; routes?: Route[] };
  if (ev.type !== "routes-changed" || !ev.routes) return;
  routes.value = ev.routes;
});
</script>
    ...
    <section class="card-base p-4">
      ...
      <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
        ...
        <label class="block xl:col-span-3">
          ...
          <p class="mt-1 text-[11px] text-vp-muted leading-snug">Preferred first.</p>
        </label>
        <label class="block xl:col-span-3">
