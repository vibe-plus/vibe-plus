...
function kindLabel(kind: SmartIntakeItem["kind"]): string {
  ...
  if (kind === "remote-provider") return "Remote";
...
<template>
  <div
    class="fixed bottom-[5.75rem] right-4 z-50 flex max-w-[calc(100vw-2rem)] flex-col items-end gap-2 sm:bottom-4"
  >
    <div
      v-if="dragActive"
    ...
    >
      ...
      <div class="mt-2 flex items-center justify-end gap-1.5">
        ...
        <button
          ...
          type="button"
          title="intake:open"
          aria-label="intake:open"
          :disabled="busy"
          @click="emit('openIntakeWizard')"
        >
          <VpIcon name="zap" size-class="size-3.5" />
        </button>
        <button
