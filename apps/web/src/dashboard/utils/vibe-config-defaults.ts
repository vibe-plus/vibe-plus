...
/** API / manual JSON may convert fields like ports into strings, which makes `Number.isInteger` misjudge them. */
function int_from_unknown(raw: unknown, fallback: number): number {
...
function port_from_unknown(raw: unknown, fallback: number): number {
  const n = int_from_unknown(raw, fallback);
  if (n >= 1 && n <= 65535) return n;
  return fallback;
}

/** Write valid integers in place to avoid permanent validation failure on the settings page. */
export function coerce_server_and_failover_numbers(cfg: VibeConfig, fb: VibeConfig): void {
  cfg.server.port = port_from_unknown(cfg.server.port, fb.server.port);
  cfg.failover.failure_threshold = int_from_unknown(
    cfg.failover.failure_threshold,
    fb.failover.failure_threshold,
  );
  cfg.failover.success_threshold = int_from_unknown(
    cfg.failover.success_threshold,
    fb.failover.success_threshold,
  );
  cfg.failover.open_timeout_secs = int_from_unknown(
    cfg.failover.open_timeout_secs,
    fb.failover.open_timeout_secs,
  );
}
...
export function normalizeVibeConfig(config: VibeConfig): VibeConfig {
  ...
  const merged: VibeConfig = {
    ...
    codex: {
      route_status_enabled:
        config.codex?.route_status_enabled ?? fallback.codex!.route_status_enabled,
      summary: {
  ...
  coerce_server_and_failover_numbers(merged, fallback);
  return merged;
}
